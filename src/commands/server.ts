...
export async function listServersCommand(): Promise<void> {
  ...
  console.log(`${'Name'.padEnd(15)} ${'Host'.padEnd(20)} ${'User'.padEnd(12)} ${'State'.padEnd(12)} ${'Apps'.padEnd(6)}`);
  console.log('-'.repeat(80));
...
export async function removeServerCommand(name: string, force = false): Promise<void> {
  ...
  if (!force) {
    console.log(`Are you sure you want to remove server '${name}'? This will not delete the actual server, only the local configuration.`);
